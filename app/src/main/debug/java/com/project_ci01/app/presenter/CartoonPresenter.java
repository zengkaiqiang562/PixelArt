...
public class CartoonPresenter extends FragmentPresenter<CartoonFragment, ImageEntityNew> {


    public CartoonPresenter(CartoonFragment fragment) {
        super(fragment);
        TAG = "CartoonPresenter";
    }

    @Override
    protected void requestData(int pageNum, int itemCount) {
        ImageDbManager.getInstance().queryByCategory(Category.CARTOON.catName, pageNum, itemCount, (totalSize, result) -> {
