...
public class PingRunnable implements Runnable {
    ...
    private static final String TAG = PingRunnable.class.getSimpleName();
    ...
    private PingBufferBuilder mPingBufferBuilder;
    ...
    public interface PingListener {
         ...
         * @param index
         */
        void onPingException(Exception e, int index);
    }
     ...
     * @param listener
     */
    public PingRunnable(final InetAddress dest, final PingListener listener) {
        mDest = dest;
        if (listener == null) {
            throw new NullPointerException();
        }
        mListener = listener;
        final byte type = dest instanceof Inet6Address ? PingBufferBuilder.TYPE_ICMP_V6 : PingBufferBuilder.TYPE_ICMP_V4;
        setPingBufferBuilder(new PingBufferBuilder(type, "abcdefghijklmnopqrstuvwabcdefghi".getBytes()));
    }
    ...
    public void setPingBufferBuilder(final PingBufferBuilder pingBufferBuilder) {
        mPingBufferBuilder = pingBufferBuilder;
    }
            ...
            if (fd.valid()) {
                try {
                    ...
                    for (int i = 0; i < mCount; i++) {
                        final ByteBuffer byteBuffer = mPingBufferBuilder.build();
