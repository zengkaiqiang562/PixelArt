...
public class TestExecChartHelper {

    private static final String TAG = "TestExecChartHelper";
    private static final String LABEL_RX = "RX";
    private static final String LABEL_TX = "TX";

    public static void updateRxChartView(Context context, LineChart rxChart) {

        List<TrafficRateBean> rateBeans = TrafficChartManager.getInstance().getTrafficRateBeans();

        if (rateBeans.isEmpty()) {
            rxChart.setVisibility(GONE);
            return;
        }

        rxChart.setVisibility(VISIBLE);

        initRxChart(context, rxChart, rateBeans);


        LineData lineData = rxChart.getLineData();
...
//            txLineDataSet.setEntries(txDataSet.getEntries());
            ...
            rxChart.notifyDataSetChanged();
        } else {
            ArrayList<ILineDataSet> dataSets = new ArrayList<>(1); // 一条RX数据链
            dataSets.add(rxDataSet);
//            dataSets.add(txDataSet);
            lineData = new LineData(dataSets);
            rxChart.setData(lineData);
//        chart.invalidate();
        }
    }

    public static void updateTxChartView(Context context, LineChart txChart) {

        List<TrafficRateBean> rateBeans = TrafficChartManager.getInstance().getTrafficRateBeans();

        if (rateBeans.isEmpty()) {
            txChart.setVisibility(GONE);
            return;
        }

        txChart.setVisibility(VISIBLE);

        initTxChart(context, txChart, rateBeans);


        LineData lineData = txChart.getLineData();
        ...
        if (lineData != null && lineData.getDataSetCount() > 0/*lineData.getDataSetByLabel(LABEL_RX, false) != null && lineData.getDataSetByLabel(LABEL_TX, false) != null*/) {
            ...
            txLineDataSet.setEntries(txDataSet.getEntries());
            lineData.notifyDataChanged();
            txChart.notifyDataSetChanged();
        } else {
            ArrayList<ILineDataSet> dataSets = new ArrayList<>(1); // 一条RX数据链
            dataSets.add(txDataSet);
            lineData = new LineData(dataSets);
            txChart.setData(lineData);
//        chart.invalidate();
        }
    }
...
//        }
        ...
        for (int index = 0; index < rxRateBeans.size(); index++) {
            TrafficRateBean rxRateBean = rxRateBeans.get(index);

            Calendar calendar = Calendar.getInstance();
            calendar.setTimeInMillis(rxRateBean.time);
            int ms = calendar.get(Calendar.MILLISECOND);
            int sec = calendar.get(Calendar.SECOND);
            int minute = calendar.get(Calendar.MINUTE);
            int msTime = ms + sec * 1000 + minute * 60 * 1000;
            rxEntries.add(new Entry(msTime, rxRateBean.rxRate));
...
//                rxDataSet.setHighLightColor(colorInt);//设置点击交点后显示交高亮线的颜色
    ...
    private static void initRxChart(Context context, LineChart rxChart, @NonNull List<TrafficRateBean> rateBeans) {
        //禁止与图表的所有可能的触摸交互
        rxChart.setTouchEnabled(false);
        ...
        rxChart.setDescription(description); // 设置图表描述信息

        //设置图列
        Legend le = rxChart.getLegend();
        ...
        rxChart.setNoDataText(""); // 没有数据时显示的文字
        rxChart.setNoDataTextColor(Color.parseColor("#FF9D9D9D")); // 没有数据时显示文字的颜色
        rxChart.setDrawGridBackground(false); //chart 绘图区后面的背景矩形将绘制
        rxChart.setDrawBorders(false); // 禁止绘制图表边框的线
        rxChart.setScaleEnabled(false);

        rxChart.animateXY(0, 0); // 去掉绘制图表时的动画（从左到右，从下到上）
        rxChart.setMinOffset(0); // 去掉图表的边距（默认15dp）

        // x轴线
        XAxis xAxis = rxChart.getXAxis();
...
//        });
        ...
        // 左轴
        YAxis yAxis = rxChart.getAxisLeft();
        ...
        float minRate = TrafficChartManager.getInstance().minRxRate();
        float maxRate = TrafficChartManager.getInstance().maxRxRate();
...
//        });
        ...
        // 右轴
        YAxis rightYAxis = rxChart.getAxisRight();
    ...
    private static void initTxChart(Context context, LineChart txChart, @NonNull List<TrafficRateBean> rateBeans) {
        //禁止与图表的所有可能的触摸交互
        txChart.setTouchEnabled(false);
        ...
        description.setTextColor(Color.RED);
        description.setTextSize(20);
        txChart.setDescription(description); // 设置图表描述信息

        //设置图列
        Legend le = txChart.getLegend();
        le.setOrientation(Legend.LegendOrientation.HORIZONTAL); // 图列水平排列
        le.setTextColor(Color.parseColor("#FF9D9D9D"));
        le.setTextSize(12);
        le.setForm(Legend.LegendForm.CIRCLE);
        ...
        txChart.setNoDataText(""); // 没有数据时显示的文字
        txChart.setNoDataTextColor(Color.parseColor("#FF9D9D9D")); // 没有数据时显示文字的颜色
        txChart.setDrawGridBackground(false); //chart 绘图区后面的背景矩形将绘制
        txChart.setDrawBorders(false); // 禁止绘制图表边框的线
        txChart.setScaleEnabled(false);

        txChart.animateXY(0, 0); // 去掉绘制图表时的动画（从左到右，从下到上）
        txChart.setMinOffset(0); // 去掉图表的边距（默认15dp）

        // x轴线
        XAxis xAxis = txChart.getXAxis();
        xAxis.setDrawLabels(false); // 显示 或 隐藏
        xAxis.setDrawGridLines(false);
        xAxis.setDrawAxisLine(false); // 隐藏x轴线
        xAxis.setAxisLineColor(Color.parseColor("#FFC6C6C6"));
...
//        });
        ...
        // 左轴
        YAxis yAxis = txChart.getAxisLeft();
        yAxis.setDrawLabels(false); // 显示 或 隐藏左轴坐标值
        yAxis.setDrawAxisLine(false); // 隐藏左轴线
        yAxis.setAxisLineColor(Color.parseColor("#FFC6C6C6"));
        yAxis.setTextColor(Color.parseColor("#FF8C857E"));
        ...
        float minRate = TrafficChartManager.getInstance().minTxRate();
        float maxRate = TrafficChartManager.getInstance().maxTxRate();
        float minY = minRate * 0.8f;
        float maxY = maxRate * 1.2f;
        yAxis.setAxisMinimum(minY);
        yAxis.setAxisMaximum(maxY);
...
//        });
        ...
        // 右轴
        YAxis rightYAxis = txChart.getAxisRight();
        rightYAxis.setDrawLabels(false); // 显示 或 隐藏
        rightYAxis.setDrawAxisLine(false); // 右轴线
        rightYAxis.setDrawGridLines(false);
        rightYAxis.setDrawTopYLabelEntry(false);
