...
import com.project_m1142.app.base.advert.AdResourceManager;
import com.project_m1142.app.base.advert.PlaceType;
...
import com.project_m1142.app.databinding.ActivityStartBinding;
import com.project_m1142.app.ui.constants.IntentConstants;
import com.project_m1142.app.ui.helper.StartHelper;
import com.project_m1142.app.base.user.UserService;
import com.project_m1142.app.base.user.User;
...
public class StartActivity extends AdResourceActivity implements IEventListener {
    ...
    private static final int WHAT_LOOP_TEXT = 5002;

    private static final String[] sLoopTexts = new String[] {
            "Individual Configuring...",
    ...
    private StartHelper startHelper = new StartHelper();
...
//        AdResourceManager.INSTANCE.pullAds();
    ...
    private void init(Intent intent) {
        ...
        startHelper.setReadyProgressListener(new StartHelper.ReadyProgressListener() {
            ...
            public void onChanged(int progress) {
                ...
                binding.startAnim.setProgress(fProgress);
                String fitProgress = "(" + progress + "%)";
                binding.startProgress.setText(fitProgress);
            }
    ...
    protected void onStart() {
        super.onStart();
        boolean agreed = SPUtils.getInstance().getBoolean(SPConstants.SP_AGREE_PROTOCOL, false);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == IntentConstants.REQUEST_CODE_PRIVACY) {
...
//        }
    ...
    private void startPrivacyActivity() {
        if (checkTurnFlag() && !privacyShowing) {
            ...
            startActivityForResult(intent, IntentConstants.REQUEST_CODE_PRIVACY);
            privacyShowing = true;
        }
    }
    ...
    private void turn2Next(long delay) { // delay: ms
        ...
        uiHandler.postDelayed(() -> {
            ...
            if (!ContextManager.INSTANCE.contains(PrivacyActivity.class)
                    && !ContextManager.INSTANCE.contains(ConnectedReportActivity.class)
                    && !ContextManager.INSTANCE.contains(UnconnectReportActivity.class)
                    && !ContextManager.INSTANCE.contains(MineActivity.class)
                    && !ContextManager.INSTANCE.contains(AboutActivity.class)
                    && !ContextManager.INSTANCE.contains(WiFiActivity.class)
                    && !ContextManager.INSTANCE.contains(WifiConnectingActivity.class)
                    && !ContextManager.INSTANCE.contains(TestActivity.class)
                    && !ContextManager.INSTANCE.contains(TestHistoryActivity.class)
                    && !ContextManager.INSTANCE.contains(TestExecActivity.class)
                    && !ContextManager.INSTANCE.contains(TestResultActivity.class)
            )
//            if (true) // TODO
            { // 冷启动 or 热启动时任务栈中无首页 or 首页在栈顶则跳首页
                LogUtils.e(TAG, "--> turn2Next() turn to HomeActivity");
...
//        startRes = AdResourceManager.INSTANCE.findResource(PlaceType.START);
    ...
    public void handleMessage(@NonNull Message msg) {
        ...
        if (msg.what == WHAT_LOOP_TEXT) {
            LogUtils.e(TAG, "--> WHAT_LOOP_TEXT");
            binding.startLoopText.setText(sLoopTexts[loopIndex++%sLoopTexts.length]);
            sendLoopText(1000L);
            return;
        }
...
//        if (ready) { // 广告已准备好
    ...
    private void sendLoopText(long delay) {
        if (uiHandler.hasMessages(WHAT_LOOP_TEXT)) {
            uiHandler.removeMessages(WHAT_LOOP_TEXT);
        }
        uiHandler.sendEmptyMessageDelayed(WHAT_LOOP_TEXT, delay);
    }

    private void removeLoopText() {
        if (uiHandler.hasMessages(WHAT_LOOP_TEXT)) {
            uiHandler.removeMessages(WHAT_LOOP_TEXT);
        }
    }
